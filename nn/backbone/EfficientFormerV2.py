...
class _InputProjBackbone(nn.Module):
    ...
    def forward(self, x: torch.Tensor):
        if x.ndim != 4:
            raise ValueError(f"{self.name}: 输入必须为 4D BCHW，当前 shape={tuple(x.shape)}")
        if int(x.shape[1]) != self.in_chans:
            msg = (
                f"{self.name}: 输入通道不匹配，期望 {self.in_chans}ch，实际 {int(x.shape[1])}ch。"
                "请检查 YAML 的模态路由标记（RGB/X/Dual）与实际输入是否一致。"
            )
            LOGGER.error(msg)
            raise RuntimeError(msg)
        if self.expected_imgsz is not None:
            h, w = int(x.shape[2]), int(x.shape[3])
            if h != self.expected_imgsz or w != self.expected_imgsz:
                msg = (
                    f"{self.name}: 当前主干按 resolution={self.expected_imgsz} 构建，仅支持输入 {self.expected_imgsz}×{self.expected_imgsz}；"
                    f"实际为 {h}×{w}。"
                    "该主干内部包含与分辨率相关的预计算（不支持自动 resize/插值）。"
                    "请将训练/推理 imgsz 与 resolution 对齐（例如都设为 640 或都设为 320）。"
                )
                LOGGER.error(msg)
                raise RuntimeError(msg)
        x = self.input_proj(x)
