"""
DepthGen：对外暴露的离线深度生成器。

特点：
- 直接 from ultralytics import DepthGen 即可使用
- 默认后端 Depth Anything V2，支持 vits/vitb/vitl/vitg
- 只需简单初始化与 run(source)，无需额外配置类
"""
...
def _resolve_ref_root() -> Path:
    """定位 ref/Depth-Anything-V2-main 路径。"""
    repo_root = Path(__file__).resolve().parents[4]
    ref_root = repo_root / "ref" / "Depth-Anything-V2-main"
    if not ref_root.exists():
        raise FileNotFoundError(f"未找到 Depth-Anything-V2-main 目录: {ref_root}")
    return ref_root


def _import_depth_anything():
    """动态导入 DepthAnythingV2 相关模块。"""
    ref_root = _resolve_ref_root()
    if str(ref_root) not in sys.path:
        sys.path.insert(0, str(ref_root))
    try:
        dpt = importlib.import_module("depth_anything_v2.dpt")
        transform_mod = importlib.import_module("depth_anything_v2.util.transform")
    except ModuleNotFoundError as e:  # noqa: BLE001
        raise ModuleNotFoundError(
            "未找到 depth_anything_v2 模块，请确认 ref/Depth-Anything-V2-main 完整存在"
        ) from e
    return dpt, transform_mod


def _default_weights_path(encoder: str) -> Path:
    ref_root = _resolve_ref_root()
    return ref_root / "checkpoints" / f"depth_anything_v2_{encoder}.pth"
...
class DepthGen(ModalGeneratorBase):
    """
    简单接口：DepthGen(weights=None, encoder=None, input_size=518, device=\"auto\", ...)
    ...
    def __init__(
        ...
        **kwargs,
        ...
        # encoder 允许省略；当提供 weights 时优先从权重文件名/内容自动推断
    ...
    def _gather_sources(self, source: str | Path | Iterable[str | Path]) -> List[str]:
        ...
        if isinstance(source, (str, Path)) and str(source).lower().endswith((".yaml", ".yml")):
            ...
            # 允许通过实例属性 self.split_names 筛选子集
            for split_name, split_val in splits_cfg:
                ...
                for s in split_list:
                    ...
                    # 目录推导：若 train=./images/train，则 images_root=../images，depth_root_default=../images_depth
    ...
    def load_model(self):
        dpt_mod, transform_mod = _import_depth_anything()
        self._dpt = dpt_mod
        ...
        # 1) 决定 encoder：显式指定 > 从权重名推断 > 从权重兼容性推断 > 默认 vitb
        ...
        if encoder in (None, "", "auto"):
            if weights_path is None:
                ...
                found = [k for k in model_cfgs.keys() if k in name_l]
                if len(found) == 1:
                    encoder = found[0]
                    LOGGER.info(f"[{self.method}] encoder 自动推断为 {encoder} (from weights filename)")
                elif len(found) > 1:
                    raise ValueError(f"权重文件名同时包含多个 encoder 标记 {found}，无法唯一推断：{weights_path}")
                else:
                    # 再尝试从权重内容兼容性推断（不做静默降级，无法唯一确定则抛错）
                    if not weights_path.exists():
                        raise FileNotFoundError(f"权重文件不存在: {weights_path}")
                    state = torch.load(weights_path, map_location="cpu")
                    candidates = []
                    for enc_key, cfg in model_cfgs.items():
                        try:
                            m = dpt_mod.DepthAnythingV2(**cfg).eval()
                    ...
                    if len(candidates) == 1:
                        ...
                        raise ValueError(
                            f"无法从权重推断 encoder（不兼容 vits/vitb/vitl/vitg）：{weights_path}。"
                            "请确认权重来源，或显式传入 encoder。"
                        )
                    else:
                        raise ValueError(
                            f"权重与多个 encoder 兼容 {candidates}，无法唯一推断：{weights_path}。"
                            "请显式传入 encoder 以消除歧义。"
                        )

        if encoder not in model_cfgs:
            raise ValueError(f"不支持的 encoder: {encoder}")
        self.encoder = encoder

        weights = weights_path if weights_path is not None else _default_weights_path(self.encoder)
        if not weights.exists():
            raise FileNotFoundError(f"权重文件不存在: {weights}")

        # 构建模型
        self.model = dpt_mod.DepthAnythingV2(**model_cfgs[self.encoder]).to(self.device).eval()
        if state is None:
        ...
        self._transform_list = [
            ...
            transform_mod.NormalizeImage(mean=[0.485, 0.456, 0.406], std=[0.229, 0.224, 0.225]),
            transform_mod.PrepareForNet(),
        ]

        LOGGER.info(f"[{self.method}] 模型加载完成: {self.encoder} @ {weights}")
    ...
    def infer(self, batch_inputs: List[torch.Tensor]) -> List[torch.Tensor]:
        ...
        with torch.no_grad():
            depth = self.model(batch)  # [B, H', W']
        return [d for d in depth]  # list of tensors
        ...
        for depth_np, meta in zip(outputs, metas):
            ...
            if self.save_color:
                if self.grayscale_color:
                    ...
                    # 某些环境无 COLORMAP_SPECTRAL，用 JET 兜底
            ...
            if self.save_comparison and "bgr" in meta:
                ...
                split = np.ones((bgr.shape[0], 16, 3), dtype=np.uint8) * 255
                comp = cv2.hconcat([bgr, split, depth_vis])
                if save_opt.save_dir is None and "depth_root_default" in meta:
