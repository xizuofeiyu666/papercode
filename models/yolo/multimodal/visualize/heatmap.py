...
class DetectionTarget(torch.nn.Module):
    ...
    def __call__(self, model_output):
        ...
        """
        # NOTE:
        # - Grad-CAM requires gradients from a scalar target back to target layer activations.
        # - NMS is non-differentiable and frequently breaks the gradient path (grads=None).
        # - Therefore this target MUST operate on raw model outputs (pre-NMS) to keep gradients valid.

        def _first_tensor(x: Any) -> Optional[torch.Tensor]:
            if isinstance(x, torch.Tensor):
                return x
            if isinstance(x, dict):
                # Prefer common prediction keys if present
                for k in ("pred", "prediction", "preds", "output", "logits"):
                    v = x.get(k, None)
        ...
        output = _first_tensor(model_output)
        if output is None:
            raise ValueError("DetectionTarget: model_output 中未找到可用于 CAM 的张量输出（期望 torch.Tensor）。")

        # Unify to [B, N, D]
        if output.ndim == 2:
            output = output.unsqueeze(0)  # [1, N, D] or [1, D, N]
        if output.ndim != 3:
            raise ValueError(f"DetectionTarget: 不支持的输出维度 {output.shape}，期望 3D 张量 [B,N,D] 或 [B,D,N]。")

        # Heuristic: D dimension should be relatively small (e.g., 4+nc or 5+nc), N can be large.
        # If shape looks like [B, D, N], transpose to [B, N, D].
        if output.shape[1] <= 512 and output.shape[2] > output.shape[1]:
            output = output.transpose(1, 2)

        b, n, d = output.shape
        if d < 5:
            # Fallback: ensure scalar depends on output to keep gradients alive
            return output.abs().mean()

        boxes = output[..., :4]  # [B,N,4]
        scores = output[..., 4:]  # [B,N,?]
        # Class score proxy: max over remaining dims to keep it model-agnostic.
        cls_score = scores.max(dim=-1).values  # [B,N]

        # Build per-prediction target value
        if self.output_type == 'class':
            value = cls_score
            rank = cls_score
        elif self.output_type == 'box':
            # Normalize by a typical input size to stabilize scale
            value = (boxes / 640.0).sum(dim=-1).abs()
            rank = cls_score  # still rank by score if possible
        elif self.output_type == 'all':
            value = cls_score + (boxes / 640.0).sum(dim=-1)
            rank = cls_score
        else:
            raise ValueError(f"Unknown output_type: {self.output_type}")

        # Select top candidates per image (avoid empty selection -> disconnected gradients)
        targets: List[torch.Tensor] = []
        for bi in range(b):
            r = rank[bi]
            v = value[bi]

            # Filter by threshold; if nothing passes, fallback to all predictions
            mask = r > float(self.conf_threshold)
            if mask.any():
                r_sel = r[mask]
                v_sel = v[mask]
            else:
            ...
            num = int(r_sel.numel())
            k = max(1, int(round(num * float(self.ratio)))) if num > 0 else 1
            k = min(k, num) if num > 0 else 1

            # Top-k by rank
            if num > 0:
                top_idx = torch.topk(r_sel, k).indices
                r_top = r_sel[top_idx]
                v_top = v_sel[top_idx]
            else:
                # Should not happen, but keep grad path connected
                r_top = r[:1]
                v_top = v[:1]
            ...
            if self.weight_by_confidence:
                ...
                w = torch.ones_like(r_top)
            w = w / (w.sum() + 1e-12)
            targets.append((v_top * w).sum())
